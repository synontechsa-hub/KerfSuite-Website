...
describe('search-engine discovery metadata', () => {
  it('publishes only canonical public marketing pages', () => {
    ...
    expect(entries.map(({ url }) => url)).toEqual([
      'https://www.kerf-suite.com/',
      'https://www.kerf-suite.com/downloads',
    ]);
  ...
  it('advertises one canonical sitemap and blocks non-public routes', () => {
    ...
    expect(metadata.sitemap).toBe('https://www.kerf-suite.com/sitemap.xml');
    expect(metadata.host).toBe('https://www.kerf-suite.com');
