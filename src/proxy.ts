...
export async function proxy(request: NextRequest) {
  try {
    ...
    if ((path.startsWith('/api/v1/licenses/verify') || path.startsWith('/api/v2/entitlements/lease') || path.startsWith('/api/v2/device-authorization')) && licenseRatelimit) {
      try {
    ...
    const error = err instanceof Error ? err.message : 'Unknown proxy error'
